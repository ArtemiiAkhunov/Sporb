...
void animation() {
  bn::sprite_ptr ninja_sprite = bn::sprite_items::astronaut.create_sprite(0, 0);

  bn::sprite_animate_action<2> action = bn::create_sprite_animate_action_forever(
    ninja_sprite, 2, bn::sprite_items::astronaut.tiles_item(), 1, 0);

  while(! bn::keypad::start_pressed()) {
    if(bn::keypad::left_pressed())
    {
                action = bn::create_sprite_animate_action_forever(
                            ninja_sprite, 16, bn::sprite_items::astronaut.tiles_item(), 1, 0);
            }
            ...
            {
                action = bn::create_sprite_animate_action_forever(
                            ninja_sprite, 16, bn::sprite_items::astronaut.tiles_item(), 0, 1);
            }
